  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-black text-white p-8 lg:p-12 pb-20">
      <div className="container mx-auto space-y-12">
        ...
        {/* Filters & Search */}
        <div className="filter-section flex justify-between mb-4">
          ...
          <div className="filter-options flex space-x-4">
            <button className="bg-yellow-500 text-white px-4 py-2 rounded-lg hover:bg-yellow-400">Most Recent</button>
            <button className="bg-yellow-500 text-white px-4 py-2 rounded-lg hover:bg-yellow-400">Most Liked</button>
            <button className="bg-yellow-500 text-white px-4 py-2 rounded-lg hover:bg-yellow-400">Milestones</button>
          </div>
        </div>
            ...
            {posts.map((post) => (
              <div key={post.id} className="post-card bg-gray-800 p-4 rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-500">
                <div className="post-header flex items-center mb-4">
                  ...
                  <span className="username font-bold text-lg">{post.username}</span>
                </div>
                ...
                <div className="post-actions mt-4 flex justify-between text-sm text-gray-400">
                  <button className="like-btn flex items-center hover:text-yellow-400"><FaHeart className="mr-1" /> {post.likes}</button>
                  <button className="comment-btn flex items-center hover:text-yellow-400"><FaComment className="mr-1" /> {post.comments}</button>
                </div>
              </div>
            ))}
          </div>
